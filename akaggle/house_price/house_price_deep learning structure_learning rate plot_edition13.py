...
def neural_net_model(X_data,input_dim):
    ...
    W_O = tf.Variable(tf.random_uniform([layer_4_num,1]))
    b_O = tf.Variable(tf.zeros([1]))
    output = tf.add(tf.matmul(layer_4,W_O), b_O)
...
for learning in learning_rate_setting:
    ...
    optimizer = tf.train.GradientDescentOptimizer(learning).minimize(cost)
