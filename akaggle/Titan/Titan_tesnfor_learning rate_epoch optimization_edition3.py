...
#進入module--------------------------------------------------------
#------------------------------------------------------------------
learning_rate_setting=[0.01,0.001,0.0001,0.00001]
epoch_testing_num=[30,60,90,120,150,180,210,240]
...
output_layer = {'weights':tf.Variable(tf.random_normal([n_nodes_hy4,n_classes])),'biases':tf.Variable(tf.random_normal([n_classes]))}
...
prediction = tf.matmul(layer_4,output_layer['weights']) + output_layer['biases']
...
for learning in learning_rate_setting:
    ...
    for epoch_num in epoch_testing_num:
        ...
        #plot learning rates & epochs graph 
...
cost_history=cost_history.reshape((len(learning_rate_setting), len(epoch_testing_num)))
ja.Plot_loop(epoch_testing_num,cost_history)
...
#    submission.to_csv("submission_titan.csv", index=False)
#-----------------------------------------------------------