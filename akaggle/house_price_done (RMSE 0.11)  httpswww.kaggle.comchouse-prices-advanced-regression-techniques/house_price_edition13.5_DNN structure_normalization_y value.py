...
for learning_rate in learning_rate_setting:
    ...
    # cost history tracking
        ...
        pred_yyy=[]
        valid_yyy=[]
        pred_yy= np.exp(pred_y) 
        pred_yyy=np.append(pred_yyy, pred_yy)
        valid_yy= np.exp(valid_y) 
        valid_yyy=np.append(valid_yyy,valid_yy)    
    
        #視覺化 fit plot
#        print("finish, plot the scatter figure")
#        plt.figure(3)
#        fig, ax = plt.subplots()
#        ax.scatter(valid_yy, pred_yy)
#        ax.plot([valid_yy.min(), valid_yy.max()], [valid_yy.min(), valid_yy.max()], 'k--', lw=3)
