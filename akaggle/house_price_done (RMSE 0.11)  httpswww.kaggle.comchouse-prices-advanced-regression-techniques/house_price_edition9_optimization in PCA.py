...
for pca_num in np.arange(range_num):
    ...
    #    plt.show()
        ...
        #if 判斷式，判斷讓pca參數跑然後逼近最佳值  

        if  pca_num == 1:
                pca_ratio=float(pca_ratio)
                pca_ratio=pca_ratio-0.5*delta
                
        elif pca_num > 1:              
            if rmse_history[-1]>  cost_history[-2]:
                pca_ratio=float(pca_ratio)
                pca_ratio=pca_ratio+0.5*delta
                if pca_ratio >=0.99999:
                    pca_ratio=0.99999
                

            
            elif rmse_history[-1]<  cost_history[-2]:
                pca_ratio=float(pca_ratio)
                pca_ratio=pca_ratio-0.5*delta         
                if pca_ratio <=0:
                    pca_ratio=0.1
