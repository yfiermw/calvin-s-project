...
n_row = 0
text = open('C:/Users/ccalvin97/.spyder-py3/machinelearningcourse/train_pm2.5.csv', 'r', encoding='big5') 
row = csv.reader(text)
for r in row:
    # 第0列沒有資訊
    if n_row != 0:
...
# add bias
x = np.concatenate((np.ones((x.shape[0],1)),x), axis=1) #x.shape[0]為第一為長度
#shape[1]為第二維長度以此類推

w = np.zeros(len(x[0]))
l_rate = 1000000000000000000
repeat = 10000
...
test_x = []
n_row = 0
text = open('C:/Users/ccalvin97/.spyder-py3/machinelearningcourse/test_pm2.5.csv' ,"r")
