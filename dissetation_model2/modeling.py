...
def regression_loss(mode,hidden, labels,n_class, initializer, scope, reuse=None,
                    return_logits=False):
#     if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:    
    with tf.variable_scope(scope, reuse=reuse):
        logits = tf.layers.dense(  ## output shape should be (2,31)??
            hidden, ## shape=(2, 1024)
            31,
            kernel_initializer=initializer,
            name='logit')

    bool_finite = is_finite(labels)
    labels = boolean_mask(labels, bool_finite)
    logits = boolean_mask(logits, bool_finite)
    labels = tf.cast(labels, tf.float32)
    logits = tf.cast(logits, tf.float32)
    loss= tf.sqrt(tf.losses.mean_squared_error(labels,logits))
    if return_logits:
        return loss, logits
...
# def regression_loss_prediction(FLAGS,hidden, labels,n_class,initializer, scope, reuse=None,
#                     return_logits=False):
#     saver=tf.train.import_meta_graph('home/xlnet/exp/sts-b/' + "model.ckpt-{}.meta".format(FLAGS.train_steps))
#     graph=tf.get_default_graph()
#     weights = tf.get_default_graph().get_tensor_by_name('logit')
#     bias = tf.get_default_graph().get_tensor_by_name('logit')
